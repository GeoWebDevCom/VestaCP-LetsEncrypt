...
if (!empty($_POST['save'])) {
    ...
    if (( $v_ssl == 'yes' ) && ($v_ssl_le != 'yes') && (empty($_POST['v_ssl'])) && (empty($_SESSION['error_msg']))) {
    ...
    // Delete Let's Encrypt SSL certificate
    if (( $v_ssl == 'yes' ) && ($v_ssl_le == 'yes') && (empty($_POST['v_ssl'])) && (empty($_SESSION['error_msg']))) {
        exec (VESTA_CMD."v-delete-web-domain-ssl-letsencrypt ".$v_username." ".$v_domain." 'no'", $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        $v_ssl = 'no';
        $v_ssl_le = 'no';
    ...
    // Add SSL certificate
    if (( $v_ssl == 'no') && (v_ssl_le != 'yes') && (!empty($_POST['v_ssl'])) && (empty($_POST['v_ssl_le'])) && (empty($_SESSION['error_msg']))) {
    ...
    if (( $v_ssl == 'no') && ($v_ssl_le != 'yes') && (!empty($_POST['v_ssl'])) && (!empty($_POST['v_ssl_le'])) && (empty($_SESSION['error_msg']))) {
        ...
        if (!empty($errors[0])) {
            ...
            exec (VESTA_CMD."v-add-web-domain-ssl-letsencrypt ".$user." ".$v_domain." ".$v_ssl_home." 'yes'", $output, $return_var);
            check_return_code($return_var,$output);
            unset($output);
            $v_ssl = 'yes';
            $v_ssl_le = 'yes';
